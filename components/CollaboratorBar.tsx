...
import { colorForStudent, type FieldTypist, type PeerPresence } from "@/lib/presence";

function fieldLabel(peer: PeerPresence | FieldTypist): string {
...
export function CollaboratorBar({
  ...
  const fromTypists: PeerPresence[] = typists.map((typist) => ({
    studentId: typist.studentId,
    studentName: typist.studentName,
    stageKey: typist.stageKey,
    fieldKey: typist.fieldKey,
    fieldLabel: typist.fieldLabel,
    updatedAt: typist.updatedAt,
  }));

  const combined = [...fromTypists, ...peers];
  if (combined.length === 0) return null;

  const unique = Array.from(new Map(combined.map((peer) => [peer.studentId, peer])).values());
